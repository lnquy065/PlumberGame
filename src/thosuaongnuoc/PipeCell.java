...
public class PipeCell extends JButton {
    ...
    public int getIntPortOut() {
        int outI=0,inI=0;
         for (int i=0;i<4;i++) {
                outI*=2;
                if (Pout[i]==true) outI+=1;
                if (Pin[i]==true) inI+=1;
            }   
         if (outI==0 && inI==0) return -1;
         else return outI;
    }
    ...
    public void cellEmpty() {
        if (type==-1) setIcon(ImgDB.get("null.png"));
        else setIcon(ImgDB.get("p"+type+"e_r"+rotStt+".png"));
    }
    public void cellRun() {
        String inI="",outI="";
        if (type==5) rotStt=0;
        if (type==0) setIcon(ImgDB.get("p0r.gif"));
        else if (type!=1) {
            for (int i=0;i<4;i++) {
                if (Pin[i]==true) inI+=1;
                else inI+=0;
